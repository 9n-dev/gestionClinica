 ...
 * y los números de meses anteriores sigan cuadrando. Se van nombre, teléfono, email, notas y los emails guardados
 * de sus citas (llevan su nombre en el texto).
 */
...
export async function datosDePaciente(id: string) {
  const p = await prisma.paciente.findUnique({
    where: { id },
    include: { citas: { orderBy: { inicio: "asc" }, include: { servicio: true, profesional: true, emails: { orderBy: { enviadoAt: "asc" } } } } },
  });
  if (!p) return null;
  return {
